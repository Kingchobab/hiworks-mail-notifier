...
- 신규 메일마다 **개별 알림** 표시
- 각 알림 본문에 메일 **제목 / 발신자** 표시
- 알림 클릭 시 **해당 메일 상세 화면으로 바로 이동**
- 초기 실행 시 기존 메일로 인한 알림 폭탄 방지
- 로그인 만료로 로그인 페이지로 이동하거나 메일 목록 조회가 인증 실패하면 **로그인 필요 알림** 표시
...
- 여러 건이 한 번에 감지되면 신규 메일마다 개별 알림 표시
- 이미 알림을 보낸 메일은 로컬 저장소에 기록하여 재알림 방지
- 로그인 페이지로 이동했거나 인증 만료로 메일 목록 조회에 실패하면 로그인 필요 알림 표시
