...
// @updateURL    https://raw.githubusercontent.com/USER/REPO/master/hiworks-mail-notifier.user.js
// @downloadURL  https://raw.githubusercontent.com/USER/REPO/master/hiworks-mail-notifier.user.js
...
(() => {
  ...
  function isSeen(no) {
    ...
    function notify(title, text) {
        // 1) Web Notification (가장 오래 유지됨)
        if ('Notification' in window) {
            if (Notification.permission === 'granted') {
                new Notification(title, {
                    body: text,
                    requireInteraction: true, // ⭐ 핵심
                    silent: false,
                });
                return;
            }
